...
func _activate() -> void:
	...
	# mid-activation) — no is_instance_valid(self) guard needed since
	# nothing below touches self (unlike dialog_marker.gd's cutscenes,
	# which do read/write self after their own equivalent wait).
